...
class _TimeRowState extends State<TimeRow> {
  ...
  DateTime _timeCalculator(int step, int index, Duration dif) {
    ...
    DateTime? _time;
    if (candleNumber < 0) {
      if (widget.candles.length <= step + candleNumber) {
        _time = widget.candles[widget.candles.length - 1].date.add(dif);
      } else {
      ...
      if (widget.candles.length > newIndex) {
        _time = widget.candles[widget.candles.length - 1].date
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.candles.length < 2) {
      dif = Duration(minutes: 5);
    } else if (step > widget.candles.length-1) {
      dif = widget.candles[0].date
          .difference(widget.candles[widget.candles.length - 1].date);
    } else {
      dif = widget.candles[0].date.difference(widget.candles[step].date);
    }
    return Padding(
